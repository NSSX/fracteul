...
int ft_zoom(int mouse, int x, int y, t_struct *mystruct)
{
  ...
  mystruct->j->xmax = (mystruct->j->xmax - x_zoom) * i + x_zoom;//faute
...
void julia_init(t_struct *mystruct)
{
  ...
  mystruct->j->ymin = -2;
  mystruct->j->ymax = 2;
  mystruct->j->cx = -0.7;//-0.8;
  mystruct->j->cy = 0.2;//1.2;
  mystruct->j->ite = 50;
  myjulia(mystruct);
}
      ...
      while (mystruct->j->y < mystruct->j->zoom_y)
        {
          ...
          mystruct->j->y2 = 0;
          mystruct->j->i = 0;
          while ((mystruct->j->x2 + mystruct->j->y2 <= 4) && (mystruct->j->i < mystruct->j->ite))
            {
              julia_give(mystruct->j);
            }
          if(ok_draw2(mystruct->j->x , mystruct->j->y))
            my_pixel_put_to_image(mystruct->img, mystruct->j->x , mystruct->j->y, RGB(mystruct->j->i*210/mystruct->j->ite,0, mystruct->j->i*210/mystruct->j->ite));
          mystruct->j->y++;
        }
      mystruct->j->x++;
    }
